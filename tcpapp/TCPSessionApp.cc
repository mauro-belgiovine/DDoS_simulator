...
void TCPSessionApp::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        if(strcmp(script, "generate") == 0){
            std::string script = generateScript();
            std::cout << script << std::endl;   //debug
            parseScript(script.c_str());
        }else parseScript(script);
...
void TCPSessionApp::parseScript(const char *script)
{
    ...
    cModule *calleeModule = getParentModule()->getParentModule()->getSubmodule("server")->getSubmodule("tcpApp", 0);

    TCPSinkApp *callee = check_and_cast<TCPSinkApp*>(calleeModule);
    callee->addTotalBytes(total);
...
// implementata da noi:
std::string TCPSessionApp::generateScript(){
    std::string final;
    double time;
    int bytes;
    double step = 0;
    ...
    for (int i=0; i<numSend; i++){
        time = uniform(step, (step+0.5)  );
        time = floor(time * 10.0f) / 10.0f;
        if (step>4) {
            int x = intuniform (1,10);
            if (x>7) bytes = intuniform (2000, 10000);
            else bytes = intuniform (10000, 30000);
        }
        else bytes = intuniform (2000, 8000);
        total += bytes;
        step += 0.5;
        std::ostringstream s;
        s << time<<" "<<bytes<<";";
