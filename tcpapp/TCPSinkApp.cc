...
simsignal_t TCPSinkApp::clientEndTransm = registerSignal("clientEndTransm");
...
void TCPSinkApp::handleMessage(cMessage *msg)
{
    ...
    else if (msg->getKind() == TCP_I_DATA || msg->getKind() == TCP_I_URGENT_DATA)
    {
        ...
        if(bytesRcvd == totalBytes) {
            emit(clientEndTransm, simTime());
            std::cout << "[CLIENT] FINISHED AT T=" << simTime() << std::endl;
        }
...
void TCPSinkApp::addTotalBytes(int bytes){
